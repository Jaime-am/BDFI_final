...
def classify_flight_delays_realtime_response(unique_id):
  ...
  query = f"SELECT * FROM flight_delay_classification_response WHERE uuid = {UUID(unique_id)}"
  results = session.execute(query)

  response = {"status": "WAIT", "id": unique_id, "prediction": None}
  result_data = []
  if results:
    for row in results:
        result_data.append({
  ...
  if result_data:
      response["status"] = "OK"
      response["id"] = result_data[0]["uuid"]
      response["prediction"] = result_data[0]
