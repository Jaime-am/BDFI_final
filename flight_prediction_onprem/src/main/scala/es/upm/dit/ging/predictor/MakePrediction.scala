...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val base_path= "/home/kali/practica_big_data_2023_docker"
    ...
    val finalPredictions = predictions.drop("indices").drop("values").drop("rawPrediction").drop("probability").withColumn("uuid", lower(col("UUID")))

    // Inspect the output
    finalPredictions.printSchema()

    // Explicitly select columns and ensure lowercase
    val selectedColumns = Seq("origin", "dayofweek", "dayofyear", "dayofmonth", "dest", "depdelay", "timestamp", "flightdate", "carrier", "distance", "route", "prediction", "uuid")
    ...
    val dataStreamWriter = finalPredictionsSelected
    ...
    val consoleOutput = finalPredictionsSelected.writeStream
      .outputMode("append")
      .format("console")
      .start()
